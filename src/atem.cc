...
void Atem::start() {
  ...
  //  while (active) {
  //      std::vector<unsigned char> packet = udp.recvPacket();
  //    if (packet.size() > 0) {
  //      parsePacket(packet);
  //    }
  //  }
...
void Atem::sendPacketAck() {
  // cout << "send packet for ack: " << remote_packet_id << endl;
    std::vector<uint8_t> packet(ack_packet);
...
void Atem::readCommandProductId(std::vector<uint8_t> data) {
  atem_product_id = std::string((const char*)data.data());
}

void Atem::readCommandWarning(std::vector<uint8_t> data) {
  atem_warning = std::string((const char*)data.data());
}
...
void Atem::readCommandInputProperty(std::vector<uint8_t> data) {
  ...
  if (i > 0 && i < 1000) {
    ...
    atem_input_names[i - 1] = std::string((const char*)name_data.data());

    std::vector<uint8_t> label_data(data.begin() + 22, data.begin() + 25);
    atem_input_labels[i - 1] = std::string((const char*)label_data.data());
  }
}
...
void Atem::readCommandMacroProperty(std::vector<uint8_t> data) {
  ...
  if (v > 0) {
    ...
    std::string name = std::string(data.begin() + 8, data.begin() + 8 + name_length);
...
void Atem::changeFaderPosition(uint8_t me, uint16_t source) //***
{
    if (me >= 0 && me < ctps.size() && me < nofMEs)
    {
        ctps[me] = source;
        mixEffectBlocks[me]->SetTransitionPosition(source / 1000.0); //TODO remove the 1000
        if (source == 1000)
        {
            BMDSwitcherInputId pgm = -1;
            BMDSwitcherInputId pvw = -1;
            ...
            chan_values[me * uint8_t(2)] = pvw; //swap
            chan_values[static_cast<INT16>(me) * 2 + 1] = pgm;
        }
    }
    return;
    /*
        ...
        data.push_back((source * 10) / 256);
        data.push_back((source * 10) % 256);
        //data.push_back(uint8_t((source*10) >> 0x08));
        //data.push_back(uint8_t((source*10)  & 0xff));
...
void Atem::changeProgramInput(uint8_t me, uint16_t source) {
  ...
  if (me >= 0 && me < cpgi.size() && me < nofMEs)
  {
      cpgi[me] = source;
  ...
  //Use IBMDSwitcherMixEffectBlock::SetProgramInputand
  //    IBMDSwitcherMixEffectBlock::SetPreviewInput to set the Program and Preview inputs to the mix
  //    effect block by assigning the input Ids returned by IBMDSwitcherInput::GetInputID
  //monitor->Notify(BMDSwitcherEventType::
}
...
void Atem::changePreviewInput(uint8_t me, uint16_t source) {
  ...
  //sendCommand("CPvI", data);
    if (me >= 0 && me < cpvi.size() && me < nofMEs)
    {
        cpvi[me] = source;
